...
var
  ...
  ExpenseCollectionUtil = require('../../expense_collection')(),
...
module.exports = function(sequelize){
  ...
  this.promise_calendar = function(args) {
    ...
    return Promise.join(
      ...
      Promise.try(function(){
        return this_user.getMy_expenses({
          where : {
            $and : [
              { status : { $ne : sequelize.models.Expense.status_rejected() } },
              { status : { $ne : sequelize.models.Expense.status_canceled() } },
            ],
            $and : [{
              decided_at : {
                $between : [
      ...
      function(department, company, leaves, expenses, schedule){
  ...
  // Promise all expenses requested by current user, regardless
  // their statuses
  //
  this.promise_my_expenses = function(args){
    ...
    var promise_my_expenses = this.getMy_expenses({
      // TODO here is cartesian product between expense types and users,
      // needs to be split
      include : [{
        model : sequelize.models.User,
        as    : 'user',
    ...
    // Fetch approvers for each expense in separate query, to avoid cartesian
    // products.
    .then(function(expenses){

      expenses.forEach(function(expense){ expense.user.cached_schedule = self.cached_schedule; });

      return Promise.resolve(expenses)
        .map(
          function(expense){
            return expense.promise_approver()
            .then(function(approver){
    ...
    .then(expenses => ExpenseCollectionUtil.promise_to_sort_expenses(expenses));

    return promise_my_expenses;
  };
  ...
  this.promise_my_active_expenses = function(args) {
    var year = args.year || moment.utc();

    return this.promise_my_expenses({
      year          : year,
      filter_status : [
        sequelize.models.Expense.status_approved(),
        sequelize.models.Expense.status_new(),
        sequelize.models.Expense.status_pended_revoke(),
      ],
    })
    .then(expenses => ExpenseCollectionUtil.promise_to_sort_expenses(expenses));
  };
  ...
  // Promises expenses ever booked for current user
  this.promise_my_active_expenses_ever = function() {
  ...
  // Promise expenses that are needed to be Approved/Rejected
  //
  this.promise_expenses_to_be_processed = function(){
    let self = this;
    ...
    return self
      ...
      .then(users => {
        return sequelize.models.Expense.findAll({
          include : [{
            model : sequelize.models.ExpenseType,
            as    : 'expense_type',
          },{
            model : sequelize.models.User,
            as    : 'user',
      ...
      .then( expenses => Promise
        .resolve(expenses)
        .map(
          expense => expense.user.promise_schedule_I_obey(),
          { concurrency : 10 }
        )
        .then( () => Promise.resolve(expenses) )
        .then(expenses => ExpenseCollectionUtil.promise_to_sort_expenses(expenses))
      );
  }; // END of promise_expenses_to_be_processed
  ...
  this.promise_cancelable_expenses = function(){
    var self = this;

    return self.promise_my_expenses({
      ignore_year : true,
      filter_status : [ sequelize.models.Expense.status_new() ],
    })
    .then(function(expenses){
      return Promise.map(expenses, function(expense){
        return expense.user.promise_schedule_I_obey();
      },{
        concurrency : 10,
      })
      .then(function(){ return Promise.resolve(expenses) })
      .then(expenses => ExpenseCollectionUtil.promise_to_sort_expenses(expenses));
    });
  };
