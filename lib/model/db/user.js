  ...
  return {
    ...
    reload_with_leave_details : function(args){
      ...
      return Promise.join(
        ...
        self.promise_my_active_expenses(args),
        self.getDepartment(),
        self.promise_schedule_I_obey(),
        function(leaves, expenses, department, schedule){
          self.my_leaves = leaves;
          self.my_expenses = expenses;
    ...
    models.User.addScope(
      ...
      () => ({
        include : [{
          ...
          where : {
            $and : [
              { status : { $ne : models.Expense.status_rejected() } },
              { status : { $ne : models.Expense.status_canceled() } },
            ],
          },
        }],
      })
