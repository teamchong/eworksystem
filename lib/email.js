...
const
  ...
  {getCommentsForLeave,getCommentsForExpense} = require('./model/comment');
 ...
 * Expense email
 */
Email.prototype.promise_expense_request_revoke_emails = function(args){
  const self = this,
  expense      = args.expense,
  send_mail  = self.get_send_email();

  let template_name_to_supervisor = 'expense_request_revoke_to_supervisor';
  let template_name_to_requestor  = 'expense_request_revoke_to_requestor';

  if ( expense.get('user').is_auto_approve() ) {
    template_name_to_supervisor = 'expense_request_revoke_to_supervisor_autoapprove';
    template_name_to_requestor  = 'expense_request_revoke_to_requestor_autoapprove';
  }
  ...
  const promise_email_to_supervisor = comments => self.promise_rendered_email_template({
    ...
    context : {
      ...
      requester : expense.get('user'),
      user      : expense.get('approver'),
    }
  })
  .then(email_obj =>
    send_mail({
      from    : config.get('application_sender_email'),
      to      : expense.get('approver').email,
      subject : email_obj.subject,
      html    : email_obj.body,
    })
    .then(() => expense.get('approver').record_email_addressed_to_me(email_obj))
  );
  ...
  .then(email_obj =>
    send_mail({
      from    : config.get('application_sender_email'),
      to      : expense.get('user').email,
      subject : email_obj.subject,
      html    : email_obj.body,
    })
    .then(() => expense.get('user').record_email_addressed_to_me(email_obj))
  );

  return getCommentsForExpense({expense})
    .then(comments => bluebird.join(
      promise_email_to_supervisor(comments), promise_email_to_requestor(comments),
      () => bluebird.resolve()
    ));
...
Email.prototype.promise_expense_request_emails = function(args){
  var self   = this,
  expense      = args.expense,
  send_mail  = self.get_send_email();

  var template_name_to_supervisor = 'expense_request_to_supervisor';
  var template_name_to_requestor  = 'expense_request_to_requestor';

  if ( expense.get('user').is_auto_approve() ) {
    template_name_to_supervisor = 'expense_request_to_supervisor_autoapprove';
    template_name_to_requestor  = 'expense_request_to_requestor_autoapprove';
  }
  ...
  return Promise.all([
      getCommentsForExpense({expense}),
      expense.get('user').promise_allowance(),
    ])
    .then(([comments, requesterAllowance]) => bluebird.join(
      promise_email_to_requestor({comments, requesterAllowance}),
      expense.get('user')
...
Email.prototype.promise_expense_request_decision_emails = function(args){
  const
    self   = this,
    expense  = args.expense,
    action = args.action,
  ...
  const promise_email_to_supervisor = comments => self.promise_rendered_email_template({
    template_name : 'expense_request_decision_to_supervisor',
    context : {
      expense,
      action,
      was_pended_revoke,
  ...
  const promise_email_to_requestor = comments => self.promise_rendered_email_template({
    template_name : 'expense_request_decision_to_requestor',
    context : {
      expense,
      action,
      was_pended_revoke,
...
Email.prototype.promise_expense_request_cancel_emails  = function(args){
  const
    self      = this,
    expense     = args.expense,
    send_mail = self.get_send_email();
  ...
  const promise_email_to_supervisor = comments => self.promise_rendered_email_template({
    template_name : 'expense_request_cancel_to_supervisor',
    context : {
      expense,
      comments,
      approver:  expense.get('approver'),
  ...
  .then(emailObj => send_mail({
    ...
    .then(() => expense.get('approver').record_email_addressed_to_me(emailObj))
  );

  const promise_email_to_requestor = comments => self.promise_rendered_email_template({
    template_name : 'expense_request_cancel_to_requestor',
    context : {
      expense,
      comments,
      approver:  expense.get('approver'),
  ...
  .then(emailObj => send_mail({
    ...
    .then(() => expense.get('user').record_email_addressed_to_me(emailObj))
  );
