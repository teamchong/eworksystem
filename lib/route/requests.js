...
var express   = require('express'),
    ...
    ExpenseCollectionUtil = require('../model/expense_collection')(),
...
router.get('/', function(req, res){
    ...
    Promise.join(
        ...
        req.user
          .promise_my_active_expenses_ever()
          .then(expenses => ExpenseCollectionUtil.promise_to_group_expenses(expenses)),
        req.user.promise_expenses_to_be_processed(),
        function(my_leaves_grouped, to_be_approved_leaves, my_expenses_grouped, to_be_approved_expenses){

            res.render('requests',{
                my_leaves_grouped     : my_leaves_grouped,
                to_be_approved_leaves : to_be_approved_leaves,
                my_expenses_grouped     : my_expenses_grouped,
                to_be_approved_expenses : to_be_approved_expenses,
            });
        }
    );
});
...
function expense_request_action(args) {
    var
      current_action      = args.action,
      expense_action_method = args.expense_action_method,
      was_pended_revoke   = false;
    ...
    Promise.try(function(){
      return req.user.promise_expenses_to_be_processed();
    })
    .then(function(expenses){
       var expense_to_process = _.find(expenses, function(expense){
          return String(expense.id) === String(request_id)
            && (expense.is_new_expense() || expense.is_pended_revoke_expense());
       });

       if (! expense_to_process) {
         throw new Error('Provided ID '+request_id
           +'does not correspond to any expense requests to be '+current_action
           +'ed for user ' + req.user.id
          );
       }

       was_pended_revoke = expense_to_process.is_pended_revoke_expense();

       return expense_to_process[expense_action_method]({ by_user : req.user });
    })
    .then(function(processed_expense){
      return processed_expense.reload({
        include : [
          {model : req.app.get('db_model').User, as : 'user'},
          {model : req.app.get('db_model').User, as : 'approver'},
        ],
    ...
    .then(function(processed_expense){
      ...
      return Email.promise_expense_request_decision_emails({
        expense             : processed_expense,
        action            : current_action,
        was_pended_revoke : was_pended_revoke,
      })
      .then(function(){
        return Promise.resolve( processed_expense);
      });
    })
    .then(function(processed_expense){
      req.session.flash_message('Request from '+processed_expense.user.full_name()
          +' was processed');
    ...
    .catch(function(error){
      console.error('An error occurred when attempting to '+current_action
        +' expense request '+request_id+' by user '+req.user.id+' Error: '+error
      );
      req.session.flash_error('Failed to '+current_action);
      return res.redirect_with_session('../');
    });
...
router.post(
  '/reject_expense/',
  expense_request_action({
    action              : 'reject',
    expense_action_method : 'promise_to_reject',
  })
);
...
router.post(
  ...
  expense_request_action({
    action              : 'approve',
    expense_action_method : 'promise_to_approve',
  })
);
...
router.post('/cancel_expense/', function(req, res){

  var request_id = validator.trim( req.body['request'] );

  Promise.try(function(){
    return req.user.promise_cancelable_expenses()
  })
  .then(function(expenses){
     var expense_to_cancel = _.find(expenses, function(expense){
        return String(expense.id) === String(request_id);
     });

    if ( ! expense_to_cancel ) {
      throw new Error('Given expense request is not amoung those current user can cancel');
    }

    return Promise.resolve(expense_to_cancel);
  })
  .then(function(expense){
    return expense.promise_to_cancel()
      .then(function(){ return Promise.resolve(expense)});
  })
  .then(function(expense){
  ...
  .then(function(expense){
    ...
    return Email.promise_expense_request_cancel_emails({
      expense : expense,
    })
    .then(function(){
  ...
  .then(function(){
    req.session.flash_message('The expense request was canceled');
  })
  .catch(function(error){
    console.log('An error occurred: '+error);
    req.session.flash_error('Failed to cancel expense request');
  })
  .finally(function(){
    return res.redirect_with_session('/requests/');
  });
...
router.post(
  ...
  function(req, res){
    ...
    if (!validator.isNumeric(request_id)){
      req.session.flash_error('Failed to revoke expense request');
    }

    if ( req.session.flash_has_errors() ) {
      console.log(
        'Got validation errors when revoking expense request for user ' + req.user.id
      );
    ...
    Promise
      // Get the Expense object for submitted ID
      .try(() => req.app.get('db_model').Expense.findOne({ where : { id : request_id }}))

      // Ensure that current user can act on this Expense object
      .then(requested_expense => {

        // Case when requested Expense is originated from current user
        if ( String(requested_expense.userId) === String(req.user.id) ) {
          return Promise.resolve( requested_expense )
        }

        // Case when requested Expense is originated from one of employees
        // current user can manage
        return req.user
          .promise_users_I_can_manage()
          .then(users => {
            if ( users.find(u => String(u.id) === String(requested_expense.userId)) ) {
              return Promise.resolve( requested_expense );
            }
      ...
      .then(expense_to_process => {

        // Ensure that Expense is in status from it could be revoked
        if (! expense_to_process) {
          throw new Error('Provided ID '+request_id
            +' does not correspond to any expense requests to be revoked by user '
            + req.user.id
           );
        }

        // Do the action
        return expense_to_process.promise_to_revoke();
      })

      // Ensure that Expense object has all content necessary for sending emails
      .then(processed_expense => processed_expense.reload({
        include : [
          {model : req.app.get('db_model').User, as : 'user'},
          {model : req.app.get('db_model').User, as : 'approver'},
        ],
      ...
      // Send relevant emails
      .then(processed_expense => {

        var Email = new EmailTransport();

        return Email.promise_expense_request_revoke_emails({
          expense  : processed_expense,
        })
        .then(function(){
          return Promise.resolve(processed_expense);
      ...
      .then(processed_expense => {
        req.session.flash_message(
          'You have requested expense to be revoked. '
          + (
            processed_expense.user.is_auto_approve()
            ? ''
            : 'Your supervisor needs to approve it'
          )
        );
      ...
      // Deal with issues if any occurs
      .catch(error => {
        console.error('An error occurred when attempting to revoke expense request '
            +request_id+' by user '+req.user.id+' Error: '+error
        );
        req.session.flash_error('Failed to revoke expense request');
        return res.redirect_with_session('../');
